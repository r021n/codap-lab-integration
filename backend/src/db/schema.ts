...
export const investigationStepCompletions = pgTable(
  "investigation_step_completions",
  {
    id: serial("id").primaryKey(),
    userId: integer("user_id")
      .references(() => users.id, { onDelete: "cascade" })
  ...
  (table) => ({
    uniqueUserStep: uniqueIndex(
      "investigation_step_completions_user_step_idx",
    ).on(table.userId, table.stepId),
  }),
);
