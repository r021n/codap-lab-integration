import { FileText, Download, Info, Lightbulb } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
            ...
            <div className="space-y-4">
              {datasets.length === 0 ? (
                ...
                datasets.map((dataset) => (
                  <div
                    key={dataset.id}
                    className="rounded-xl border border-border/10 p-4 flex flex-col gap-3 bg-background hover:bg-rose-500/5 hover:border-rose-500/20 transition-all shadow-sm group"
                  >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="w-full h-9 text-xs border-border text-foreground hover:bg-rose-600 hover:text-white"
